...
export interface ValidationError {
  type: 'duplicate_id' | 'duplicate_name' | 'missing_required' | 'invalid_format' | 'consistency_error' | 'review_mismatch';
  drugId: string;
  field: string;
  message: string;
}
    ...
    if (drug.hasOfficialInstruction === undefined) {
      ...
      if (!drug.instructionFile) {
        ...
        if (!fs.existsSync(instructionPath)) {
          ...
          try {
            const instruction = JSON.parse(fs.readFileSync(instructionPath, 'utf-8'));
            if (instruction.id && drug.id && instruction.id !== drug.id) {
              drugErrors.push({
                type: 'consistency_error',
                drugId: drug.id || 'UNKNOWN',
                field: 'instructionFile',
                message: `Instruction id mismatch: catalog=${drug.id}, instruction=${instruction.id}`
              });
            }
            if ((drug.reviewStatus ?? null) !== (instruction.reviewStatus ?? null)) {
              drugErrors.push({
                type: 'review_mismatch',
                drugId: drug.id || 'UNKNOWN',
                field: 'reviewStatus',
                message: `Review status mismatch: catalog=${drug.reviewStatus ?? 'null'}, instruction=${instruction.reviewStatus ?? 'null'}`
              });
            }
            if ((drug.needsManualReview ?? null) !== (instruction.needsManualReview ?? null)) {
              drugErrors.push({
                type: 'review_mismatch',
                drugId: drug.id || 'UNKNOWN',
                field: 'needsManualReview',
                message: `needsManualReview mismatch: catalog=${drug.needsManualReview ?? 'null'}, instruction=${instruction.needsManualReview ?? 'null'}`
              });
            }
          } catch (e) {
            drugErrors.push({
              ...
              drugId: drug.id || 'UNKNOWN',
              field: 'instructionFile',
              message: `Failed to read instruction file ${drug.instructionFile}: ${e instanceof Error ? e.message : String(e)}`
            });
          }
        }
      }
